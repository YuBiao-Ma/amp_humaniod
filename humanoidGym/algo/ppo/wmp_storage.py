...
class RolloutStorage:
    ...
    def __init__(
        ...
        self.wm_features = torch.zeros(num_transitions_per_env, num_envs, 512, device=self.device)
        ...
        # # For adaboost
        # self.prob = torch.ones(self.num_envs,1,device=self.device)
        # self.prob_mask = torch.rand(self.prob.shape,device=self.device)
    ...
    def _save_hidden_states(self, hidden_states):
        ...
        hid_e = hidden_states[2] if isinstance(hidden_states[2], tuple) else (hidden_states[2],)
        ...
        if self.saved_hidden_states_a is None:
            ...
            self.saved_hidden_states_e = [
                torch.zeros(self.observations.shape[0], *hid_e[i].shape, device=self.device) for i in range(len(hid_e))
            ]
        # copy the states
    ...
    def refresh_bootstrapping_data(self):
        self.prob = 1 - F.tanh(torch.std(self.rewards,dim=0)/torch.mean(self.rewards,dim=0))
        self.prob_mask = torch.rand(self.prob.shape,device=self.device)
        
    def init_bootstrapping_data(self):
        # prob that choose to use the est
        self.prob = torch.zeros(self.num_envs,1)
        self.prob_mask = torch.rand(self.prob.shape,device=self.device)
    
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        # shifted data batch version for smoothness
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                yield obs_batch, critic_observations_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, (
                    None,
                    None,
                ), None, rnd_state_batch, wm_feature_batch,(critic_observations_batch,next_critic_observations_batch)

    # for RNNs only
    def recurrent_mini_batch_generator(self, num_mini_batches, num_epochs=8):
        
        # prob = self.get_bootstrapping_data()
        
        padded_obs_trajectories, trajectory_masks = split_and_pad_trajectories(self.observations, self.dones)
        padded_wm_feature_trajectories, _ = split_and_pad_trajectories(self.wm_features, self.dones)
        if self.privileged_observations is not None:
        ...
        for ep in range(num_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                wm_batch = padded_wm_feature_trajectories[:, first_traj:last_traj]
                ...
                yield obs_batch, critic_obs_batch, actions_batch, values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, (
                    ...
                    hid_c_batch,
                    hid_e_batch
                ), masks_batch, rnd_state_batch, wm_batch, (obs_batch, critic_obs_batch, next_critic_obs_batch, hid_e_batch, masks_batch)
