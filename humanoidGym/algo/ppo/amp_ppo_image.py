...
from .rollout_storage_image import RolloutStorage
...
class AMPPPOImage:
    ...
    def init_storage(self, num_envs, num_transitions_per_env, actor_obs_shape, critic_obs_shape, action_shape, image_shape):
    ...
    def act(self, obs, critic_obs, amp_obs, images):
        if self.actor_critic.is_recurrent:
            self.transition.hidden_states = self.actor_critic.get_hidden_states()
        # Compute the actions and values
        self.transition.actions = self.actor_critic.act(obs, images).detach()
    ...
    def update(self,iter):  # noqa: C901
        ...
        for (
            ...
            if self.actor_critic.is_recurrent:
                ...
                hidden_states = None
            self.actor_critic.act(obs_batch,image_batch,masks=masks_batch, hidden_states=hidden_states)
            ...
            if hasattr( self.actor_critic, 'update' ) and callable(self.actor_critic.update):
                ...
                subtask_loss = self.actor_critic.subtask_loss(subtask_data,image_batch.squeeze(-1))
                loss+=subtask_loss
