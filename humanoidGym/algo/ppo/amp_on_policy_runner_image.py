...
from .amp_ppo_image import AMPPPOImage
from .actor_critic import ActorCritic, InferenceActor
from .actor_crtic_recurrent import ActorCriticRecurrent, InferenceActorLSTM
...
class AmpOnPolicyRunnerImage:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: Union[str, None] = None, device="cpu"):
        ...
        self.depth_predictor = DepthPredictor().to(self.device)
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):
        ...
        images = torch.zeros(((self.env.num_envs,) + self.env.cfg.depth.resized + (1,)), device=self.device)
        ...
        wm_obs = {
            "prop": obs[:, -self.env.cfg.env.num_single_observations:].to(self.device),
            "is_first": wm_is_first,
        }

        if(self.env.cfg.depth.use_camera):
            wm_obs["image"] = torch.zeros(((self.env.num_envs,) + self.env.cfg.depth.resized + (1,)), device=self.device)
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    actions = self.alg.act(obs, critic_obs, amp_obs,images)
                    ...
                    if (self.env.global_counter % self.wm_update_interval == 0):
                        if (self.env.cfg.depth.use_camera):
                            forward_heightmap = self.env.get_forward_map().to(self.device)
                            ...
                            wm_obs["image"][self.env.depth_index] = infos["depth"].unsqueeze(-1).to(self.device)
            ...
            if (sum_wm_dataset_size > self.depth_predictor_cfg["train_start_steps"]):
    ...
    def init_wm_dataset(self):
        ...
        if(self.env.cfg.depth.use_camera):
            self.wm_dataset["image"] = torch.zeros(((self.env.cfg.depth.camera_num_envs, int(self.env.max_episode_length / self.wm_update_interval) + 3,)
                                               + self.env.cfg.depth.resized + (1,)), device=self.device)
            self.wm_dataset["forward_height_map"] = torch.zeros(
                (self.env.num_envs, int(self.env.max_episode_length / self.wm_update_interval) + 3,
                 self.env.cfg.env.forward_height_dim), device=self.device)
