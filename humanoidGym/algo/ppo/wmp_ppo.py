...
from .wmp_storage import RolloutStorage
...
class WMPPPO:
    ...
    def act(self, obs, critic_obs, amp_obs, wm_feature):
        ...
        self.transition.actions = self.actor_critic.act(aug_obs, wm_feature).detach()
    ...
    def compute_returns(self, last_critic_obs,wm_feature):
        ...
        last_values = self.actor_critic.evaluate(last_critic_obs, wm_feature).detach()
    ...
    def update(self,iter):  # noqa: C901
        ...
        for (
            ...
            if self.actor_critic.is_recurrent:
                ...
                hidden_states = None
            self.actor_critic.act(obs_batch,wm_feature_batch,masks=masks_batch, hidden_states=hidden_states)
    
            actions_log_prob_batch = self.actor_critic.get_actions_log_prob(actions_batch)
            # -- critic
            value_batch = self.actor_critic.evaluate(critic_obs_batch, wm_feature_batch, masks=masks_batch,
            ...
            if hasattr( self.actor_critic, 'update' ) and callable(self.actor_critic.update):
                ...
                subtask_loss = self.actor_critic.subtask_loss(subtask_data,wm_feature_batch)
                loss+=subtask_loss
